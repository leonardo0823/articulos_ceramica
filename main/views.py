...
def index(request):
    ...
    if articulo:
        ...
        importe_articulo = round(articulo.cantidad_producida_en_el_mes*articulo.costo, 2)
    ...
    importe = articulos_encontrados.aggregate(importe=Sum('costo')*Sum('cantidad_producida_en_el_mes'))
    importe = round(importe['importe'], 2) if articulos_encontrados else 0

    articulos_rechazados = articulos.aggregate(rechazados=Sum('cantidad_rechazada'))
    articulos_rechazados = articulos_rechazados['rechazados'] if articulos else 0
    articulos_aceptados = articulos.aggregate(aceptados=Sum('cantidad_producida_en_el_mes'))
    articulos_aceptados = articulos_aceptados['aceptados'] - articulos_rechazados if articulos else 0

    importe_articulos_rechazados = articulos.aggregate(importe=Sum('costo') * Sum('cantidad_rechazada'))
    importe_articulos_rechazados = round(importe_articulos_rechazados['importe'], 2) if articulos else 0

    importe_articulos_aceptados = articulos.aggregate(importe=Sum('costo') * (Sum('cantidad_producida_en_el_mes') - Sum('cantidad_rechazada')))
    importe_articulos_aceptados = round(importe_articulos_aceptados['importe'], 2) if articulos else 0
...
def taller(request, pk):
    ...
    importe = articulos.aggregate(importe=Sum('costo')* Sum('cantidad_producida_en_el_mes'))
